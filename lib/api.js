...
const PainelAPI = {
  ...
  fetchFileBlob: async (fileId) => {
    ...
    // Validar URL em cache: se o blob foi revogado externamente, descartar
    const cached = PainelAPI._blobCache[fileId];
    if (cached) {
      try {
        // fetch rápido no blob: local para checar se ainda existe
        const check = await fetch(cached, { method: 'HEAD' });
        if (check.ok) return cached;
      } catch { /* blob revogado ou inacessível — segue para re-download */ }
      delete PainelAPI._blobCache[fileId];
    }
    try {
      const token = getToken();
