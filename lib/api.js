...
const PainelAPI = {
  ...
  // Gera URL que passa pelo n8n backend (que tem service account do Drive).
  // O navegador faz GET direto nessa URL, recebendo o binario com Content-Type correto.
  fileProxyUrl: (fileId) => {
    if (!fileId) return null;
    const token = getToken();
    return `${BASE_URL}?action=proxy_file&file_id=${encodeURIComponent(fileId)}${token ? '&token=' + encodeURIComponent(token) : ''}`;
  },
};
