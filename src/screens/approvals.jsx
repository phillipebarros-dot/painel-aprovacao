                ...
                {pageRows.map((c, i) => {
                  ...
                  return (
                    <tr key={c.submission_id} className={"row-action row-anim " + (isSel ? "is-selected " : "") + (isPick ? "compare-pick" : "")} style={{ animationDelay: (i * 22) + "ms" }} onClick={() => onRowClick(c)}>
                      ...
                      <td style={{ fontWeight: 500 }}>{c.cliente}{c.is_complement === 1 && <span className="pill pill-info" style={{ marginLeft: 6 }}>compl</span>}{c.rejection_count > 0 && c.status !== "approved" && <span className="pill pill-rejected" style={{ marginLeft: 6 }}>{c.rejection_count}× reprov.</span>}</td>
